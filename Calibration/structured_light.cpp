...
bool sl::decode_pattern(const std::vector<std::shared_ptr<cv::Mat>>& images,
                        cv::Mat& pattern_image,
                        cv::Mat& min_max_image,
                        cv::Size const& projector_size,
                        unsigned flags,
                        const cv::Mat& direct_light,
                        unsigned m)
{
	...
	std::cout << "Decode: " << (binary ? "Binary " : "Gray ") << (robust ? "Robust " : "") << std::endl;
	...
	for (unsigned t = 0; t < COUNT; t += 2, current++)
	{
		...
		//load images
		const cv::Mat& gray_image1 = *images[t + 0].get();
		if (gray_image1.rows < 1)
		{
			std::cout << "Failed to load " << images.at(t + 0) << std::endl;
			return false;
		}
		const cv::Mat& gray_image2 = *images[t + 1].get();
...
cv::Mat sl::estimate_direct_light(const std::vector<std::shared_ptr<cv::Mat>>& images, float b)
{
...
int sl::binaryToGray(int value)
{
	return util_binaryToGray(value);
}

inline int sl::binaryToGray(int value, unsigned offset)
{
	return util_binaryToGray(value + offset);
}
inline int sl::grayToBinary(int value, unsigned offset)
{
	return (util_grayToBinary(value, 32) - offset);
}
